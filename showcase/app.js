...
const screenshotsPath = join(process.cwd(), './showcase/assets/screenshots');
try {
...
function startService(port) {
  ...
  server.use((req, res, next) => {
    if (req.method === 'GET') {
      ...
      if (pathname === '/showcase/index.html') {
        const demoFiles = getFiles(__dirname)
          ...
          .map(filename => {
            ...
            const file = {
              screenshot: `/showcase/assets/screenshots/${bn}.png`,
              basename: bn,
  ...
  const url = `http://127.0.0.1:${port}/showcase/index.html`;
